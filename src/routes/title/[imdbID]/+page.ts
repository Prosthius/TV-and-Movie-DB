...
import { selectedTitleDetails, streamingAvailability } from '$lib/stores';
...
async function getInfo(
	...
	try {
		if (get(selectedTitleDetails).imdbID != imdbID) {
		...
		return get(selectedTitleDetails);
	} catch (error: unknown) {
		console.log(error);
		throw error;
	}
...
async function getStreamAvail(
	...
	try {
		if (get(streamingAvailability).result == undefined) {
			const res: Response = await fetch(`/api/streaming?imdbID=${imdbID}`);
			const json: StreamingAvailability = await res.json();
			streamingAvailability.set(json);
			return json;
		}
		return get(streamingAvailability);
	} catch (error: unknown) {
		console.error(error);
		throw error;
	}
...
async function getSeason(
	...
	try {
		...
		if ((json as TitleDetails).Response === 'False') throw new Error((json as TitleDetails).Error);
		return json as TitleDetails[];
	} catch (error: unknown) {
		console.log(error);
		throw error;
